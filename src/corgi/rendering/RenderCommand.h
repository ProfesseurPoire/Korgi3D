...
namespace corgi
{
	...
	public:
		...
		static void buffer_index_data(unsigned int buffer_id, const unsigned int * data, int size);
		static void buffer_index_subdata(unsigned int buffer_id, const unsigned int* data, int size);
		...
		static void initialize_texture_object(Texture::Format format,
			...
			int width, int height, Texture::DataType dt, void* data = 0);
		...
		static void vertex_attribute_pointer(unsigned id, unsigned stride, int offset, unsigned size);
