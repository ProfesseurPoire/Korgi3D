...
namespace corgi
{
	...
	void RenderCommand::check_error()
	{
		check_gl_error();
	}


	void RenderCommand::buffer_vertex_data(unsigned int index, const float* data, int size)
	{
		glBindBuffer(GL_ARRAY_BUFFER, index);
		glBufferData(GL_ARRAY_BUFFER, size, data, GL_STATIC_DRAW);
	}

	void RenderCommand::buffer_vertex_subdata(unsigned int index, const float* data, int size)
	{
		glBindBuffer(GL_ARRAY_BUFFER, index);
		glBufferSubData(GL_ARRAY_BUFFER, 0, size, data);
	}

	void RenderCommand::buffer_index_data(unsigned int index, const unsigned int* data, int size)
	{
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, size, data, GL_STATIC_DRAW);
	}

	void RenderCommand::buffer_index_subdata(unsigned int index, const unsigned int* data, int size)
	{
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index);
		glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, size, data);
	}

	void RenderCommand::vertex_attribute_pointer(unsigned id, unsigned stride, int offset, unsigned size)
	{
		glVertexAttribPointer(id, size, GL_FLOAT, GL_FALSE, stride, (void*)(offset * sizeof(GL_FLOAT)));
		check_gl_error();
	}
	...
	void RenderCommand::bind_vertex_array(unsigned id)
	{
	...
	void RenderCommand::delete_frame_buffer(unsigned id)
	{
	...
	void RenderCommand::initialize_texture_object(Texture::Format f, Texture::InternalFormat int_f,
		int width, int height, Texture::DataType dt, void* data)
	{
